...
- **Última reconciliação:** `2026-07-26T21:58:43-03:00`.
...
| DEC-20260726-003 | Revisar estrutura da timeline | CONCLUIDA | TL-R01 a TL-R10; validadores TL-005/TL-010 | validar remediação TL-005 no R7B |
...
| DEC-20260726-006 | Definir arquitetura Cultura–MOP–POPs | CONCLUIDA | `governance/ARCHITECTURE.md` | revisar novamente no R7B |
...
| DEC-20260726-018 | Remediar TL-R01 a TL-R10 | CONCLUIDA | esquema e normalização | validar nova política TL-005 no R7B |
| DEC-20260726-019 | Executar plano de 64 testes no R6 | CONCLUIDA | SHA `9cb5414412a562e17ee41759c5965343b5192220`, run `30224913937`, artifact `8638274889` | preservado como snapshot histórico |
...
| DEC-20260726-023 | Executar TL-005, TL-010, AU e RC | EM_TRATAMENTO | TL executáveis; AU/RC simulados | validar TL-005 R7B e implementar validadores formais futuramente |
| DEC-20260726-024 | Investigar mergeability do PR #2 | CONCLUIDA | API reportou mesclável | repetir somente se estado mudar |
| DEC-20260726-025 | Reconciliar PR #1 e PR #2 | CONCLUIDA | branch integrada e documentos reconciliados | manter estado pós-R6 coerente no R7B |
| DEC-20260726-026 | Preparar RC em contexto separado | CONCLUIDA | RC R6 executada em nova sessão | repetir sobre o novo candidato R7B |
...
| DEC-20260726-044 | Publicar fechamento TL-012 | CONCLUIDA | fechamento real e complemento append-only | preservar no R7B |
| DEC-20260726-045 | Executar remediação R5 integral | CONCLUIDA | run `30222472929` | resultado histórico; não substitui R6/R7B |
...
| DEC-20260726-049 | Reconciliar regressões FA-008 e FA-009 | CONCLUIDA | reteste manual multigrupo | mecanismo genérico substituído por cronologia estruturada |
| DEC-20260726-050 | Atualizar fontes contraditórias pré-R6 | SUBSTITUIDA | atualização anterior ficou obsoleta após R6 | sucessora `DEC-20260726-053` |
...
| DEC-20260726-053 | Reconciliar estado canônico pós-R6 | EM_TRATAMENTO | fontes atualizadas; R7 inicial revelou falha adicional documentada | validar ausência de estados obsoletos no R7B |
| DEC-20260726-054 | Remediar RC-R6-01 a RC-R6-04 e preparar sucessor | EM_TRATAMENTO | R7 inicial falhou no TL-005 e foi preservado; correção aplicada na integração | congelar e executar R7B |
| DEC-20260726-055 | Executar nova RC independente | BLOQUEADA | depende de candidato R7B e evidências completas | desbloqueia após CI integral favorável no mesmo SHA |
| DEC-20260726-056 | Preservar falha do R7 inicial | CONCLUIDA | PR #14, runs `30228522319`/`30228522369`, artifact `8639293479` | não alterar branch ou SHA; usar R7B |
...
| DEC-20260726-016 | Timeline diária estruturada | APROVADA_PROVISORIAMENTE | R7B e nova RC independente |
...
| PR #14 — candidato R7 inicial | FECHADO_APÓS_FALHA_TL005 | não realizado |
| Issue #3 — RC independente | ABERTA_COM_RESULTADO_R6 | não aplicável |
| Branch `integration/mop-governance-v0.1` | ATIVA_PREPARANDO_R7B | não promovida |
| Branch `test/governance-64-20260726-r6` | CONGELADA_HISTORICA | não promovida |
| Branch `test/governance-64-20260726-r7` | CONGELADA_HISTORICA_FAIL | não promovida |
| Branch R7B | A_CRIAR | não aplicável |

## Achados RC ainda abertos

| ID | Estado | Ação necessária |
|---|---|---|
| RC-R6-01 | EM_TRATAMENTO | validar reconciliação pós-R6 no R7B |
| RC-R6-02 | EM_TRATAMENTO | validar TL-005 corrigido no repositório real e nos testes adversariais |
| RC-R6-03 | EM_TRATAMENTO | validar cronologia estruturada e regressão posterior |
| RC-R6-04 | EM_TRATAMENTO | validar artifact bruto completo no R7B |
...
| RC-04 | CONCLUIDA_NO_R6 | independência atendida; repetir no R7B |
| RC-05 | EM_TRATAMENTO | validar controles no R7B |
| RC-06 | EM_TRATAMENTO | validar que a autoridade não cria gargalo |
| RC-07 | EM_TRATAMENTO | submeter candidato R7B reconciliado à RC |
...
|---|---|---|
| LEA-206 | PENDENTE | próximo gate é R7B, nova RC e decisão de fundação |
...
- bateria R7B concluída sobre HEAD congelado;
- artifact R7B completo verificado;
- RC independente R7B favorável;
